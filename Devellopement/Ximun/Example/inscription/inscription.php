...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($siret && !preg_match('/^[0-9]{14}$/', $siret)) $errors[] = "Le SIRET doit contenir 14 chiffres.";
        if ($code_naf && !preg_match('/^[0-9]{4}[A-Z]$/', $code_naf)) $errors[] = "Le code NAF doit être au format 0000A.";
        ...
        if (!empty($errors)) {
            ...
            echo "</ul>";
            echo "</div>";
            logEvent("Erreurs de validation: " . implode(", ", $errors));
        } else {
    ...
    echo "<a href='inscription_retour.html' class='btn'>Retour au formulaire d'inscription</a>";
...
CREATE TABLE `user` (
  ...
  UNIQUE KEY `adresse_mail` (`adresse_mail`)
            ...
            $requiredColumns = ['id_user', 'nom', 'prenom', 'adresse_mail', 'mdp'];
